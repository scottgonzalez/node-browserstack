		...
		this.request({
			...
			function reduce( arr ) {
				if( Array.isArray( arr )){
					return [].concat.apply( [], arr.map( reduce ) );
				} else {
					return arr;
				}
			}

			fn( null, reduce(Object.keys( osBrowsers ).map(function( os ) {
				return Object.keys( osBrowsers[ os ] ).map(function( osVersion ) {
					return osBrowsers[ os ][ osVersion ].map(function( browser ) {
						if ( browser.devices ){
							return browser.devices.map(function( device ){
								return {
