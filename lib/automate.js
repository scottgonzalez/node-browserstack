...
extend(AutomateClient.prototype, {
	...
	getProject: function(id, fn) {
		this.request({
			...
			if (error) {
				fn(error);
			} else if (typeof response.project === "undefined") {
				fn(new Error("This project object is not a standard response."));
			} else {
	...
	getSession: function(id, fn) {
		this.request({
			...
			if (error) {
				fn(error);
			} else if (typeof response.automation_session === "undefined") {
				fn(new Error("This session object is not a standard response."));
			} else {
